...
The four definitions measure 1,062 tokens in the client's system prompt, paid on every single turn
...
meant to avoid reading. Pinpointing inside a hit is `get`'s job too: pass `find` and the second hop
returns the passage instead of the document.
