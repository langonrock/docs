...
| `book-coarse`      | Four novels, one concept per novel   |    64% | 100% vs 100%     |
| `scripture`        | The Bible, one concept per chapter   |    53% | 90% vs 90%       |
| `reference`        | The warehouse catalogue above        |    52% | **75%** vs 70%   |
| `book`             | Four novels, one concept per chapter |     2% | 80% vs 80%       |

The store meets or beats the raw files on every profile except the novels' top position (MRR 0.69
against 0.72). On prose with no headings and no links the saving is about two percent: the
advantage does not come from having a store, it comes from documents that were already structured.
A folder of chapters is better served by reading the files.
